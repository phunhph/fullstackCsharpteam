...
public class StaffDAO
{
    ...
    public Staff SelectByID(String id)
    {
        ...
        using (SqlConnection dbConnection = new SqlConnection(connString))
        {
            ...
            {
                int timeout = 30;
                string commandText = "SELECT * FROM NhanVien where status='active' and ID_NV= @ID_NV ";
                SqlCommand command = new SqlCommand(commandText, (SqlConnection)transaction.Connection, (SqlTransaction)transaction);
                command.CommandTimeout = timeout;
                // load
    ...
    //=============================================== Edit function ===================================================//
    public bool Edit(Staff staff , String id_nv)
    {
        ...
        using (SqlConnection dbConnection = new SqlConnection(connString))
        {
            ...
            try
            {
                ...
                command.CommandTimeout = timeout;
                command.Parameters.AddWithValue("@ID_NV", id_nv);
                command.Parameters.Add("@HoTen", SqlDbType.VarChar).Value = staff.Namenv;
                command.Parameters.Add("@Gioitinh", SqlDbType.VarChar).Value = staff.Gender;
